...
const addOne = (arr) => {
  // Solution code here...
  const arrayOfAddOn = arr.map(number => {
      return number + 1;
  });
  return arrayOfAddOn;
}
...
const addQuestion = (arr) => {
  // Solution code here...
  const arrayOfAddQuestion = arr.map(string => {
      return string + '?';
  });
  return arrayOfAddQuestion;
}
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let x = 0; x < arr.length; x++) {
      powerArray.push(Math.pow(2, arr[x]));
  }
  return powerArray;
};
